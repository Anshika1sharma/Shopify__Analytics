...
const MonthlySalesGrowthChart = () => {
  const [data, setData] = useState({ labels: [], datasets: [] });

  useEffect(() => {
    const fetchGrowthData = async () => {
      try {
        const response = await fetch('http://localhost:5000/monthlySalesGrowthRate');
        if (!response.ok) {
        ...
        const labels = result.map(item => `${item.year}-${String(item.month).padStart(2, '0')}`);
  ...
  return (
    <div className="chart-page">
      <h2 className="chart-heading">Monthly Sales Growth Rate</h2>
